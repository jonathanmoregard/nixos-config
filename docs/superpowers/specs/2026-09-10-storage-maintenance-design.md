...
Eligibility never bypasses mandatory safety checks. The worktree must live below a configured sweep root, must not be a protected/default branch or repository's own checkout, must be unlocked and present, must have clean `git status --porcelain` output, must contain no ignored content except verified Nix result symlinks, and must have no visible live process working directory inside it.

Merged-at-tip removal deletes both worktree and local branch immediately. Age-only removal deletes the worktree but preserves its branch, so committed work remains easy to recover even when no merged PR exists. Detached worktrees remain protected because no branch provides that recovery point; phase-two branch cleanup also preserves every branch whose tip anchors a detached worktree.
...
Discovery resolves each registered worktree to its owning repository. Validated bare owners, including `~/Repos/nixos-config`, are addressed with explicit `--git-dir`; this preserves `safe.bareRepository = explicit` while allowing their worktrees to participate in the sweep.

## Nix Result Links

After eligibility and all safety checks pass, the sweeper inventories ignored paths with `git ls-files`. Any ignored path other than a top-level symlink named `result` or `result-*` makes removal fail closed. It unlinks those result symlinks only when `readlink` reports an absolute `/nix/store/*` target. A matching link with any other target also makes removal fail closed. The final `git worktree remove` stays non-force.
...
- Extend the destructive worktree harness first. New fixtures prove recent merged worktrees are removed, old unmerged worktrees lose only their worktree, dirty/live/locked/default/detached worktrees remain, detached-tip branches stay protected, arbitrary ignored content remains intact, GitHub outage still permits age-only cleanup but no PR-based branch deletion, non-GitHub age-only cleanup works, and only verified Nix result links are removed.
