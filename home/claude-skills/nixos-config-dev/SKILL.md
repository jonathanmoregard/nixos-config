...
without requesting that discovery. The `ncfg` shell function
(home/jonathan.nix) is that path pre-bound:
...
daily `nixos-worktree-sweep` into a no-op. `tests/base.nix` asserts the
anchor lifecycle AND that bare discovery is still refused, so a future
change that breaks the flow fails the gate whatever it is called.
