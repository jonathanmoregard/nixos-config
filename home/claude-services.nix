...
#
# These depend on supporting state under ~/.claude (dev-container scripts,
# GitHub App private key). On a fresh install
...
# separately.
{
  ...
  # reviewer writes into it at 03:20 (rsiDailyReview in home/jonathan-linux.nix),
  # the intake gate annotates every file in it, and /review-improvements edits
  # it. Until 2026-09-03 it had no git remote at all, so every proposal this
  # machine has ever generated lived on exactly one disk — the sink was moved
  # OUT of ~/.claude precisely so it would stop dirtying the config repo, and
  # the side effect was that it stopped being pushed anywhere with it.
  #
  # It now has a private GitHub origin, and nothing was pushing it unattended:
  # rsi-daily-review only writes files, and push-proposals.sh only runs when a
  # human runs /review-improvements. This timer is that missing half.
  #
  # WHY THE SCRIPT DOES THE CAREFUL PART. It commits whatever the sink holds —
  # correct HERE and wrong for ~/.claude, whose auto-committer was deleted on
  # purpose (see the crontab block in home/jonathan-linux.nix): ~/.claude's
  # working tree IS the running configuration, while the sink is a store nothing
  # reads to decide behaviour. It runs gitleaks and fails CLOSED before every
  # push, because proposals are generated by a model reading session transcripts
  # and can quote a credential that was on screen. It takes the intake gate's
  # own lock so a tick cannot commit a half-annotated proposal. And it never
  # forces, rebases or resets — a diverged remote is reported and left alone.
  # All of that is exercised by tests/test_claude_proposals_push.sh in the
  # ~/.claude repo; tests/base.nix covers what belongs to the unit.
  #
  # GITLEAKS= IS NOT DECORATION. The script refuses to push when it cannot
  # scan, so a unit that cannot find gitleaks is a unit that silently never
  # pushes. A systemd user unit does not inherit the login shell's PATH, and
  # gitleaks lives in the per-user nix profile rather than in
  # /run/current-system/sw/bin, so the store path is pinned here at build time
  # rather than left to `command -v`. It is an absolute path for the reason
  # claude-marketplace-pull.service documents two units up: systemd expands
  # specifiers in ExecStart= but NOT in Environment=.
  systemd.user.services.claude-proposals-push = {
    Unit = {
      Description = "Commit, secret-scan and push the RSI proposals sink to its private remote";
      After = [ "default.target" ];
    };
    Service = {
      Type = "oneshot";
      ...
      Environment = [ "GITLEAKS=${pkgs.gitleaks}/bin/gitleaks" ];
      # Background maintenance — never compete with an interactive session.
      Nice = 10;
      IOSchedulingClass = "idle";
      # The script bounds each of its own steps (30s for the intake lock, 60s
      # fetch, 120s push); this is the backstop for a hang anywhere else, and
      # sits well under the 15-minute cadence so a wedged tick cannot overlap
      # the next one.
      TimeoutStartSec = 300;
    };
  };

  systemd.user.timers.claude-proposals-push = {
    Unit.Description = "Push the RSI proposals sink to its private remote every 15 minutes";
    Timer = {
      # Calendar rather than monotonic for the same reason as the pullers —
      # Persistent= has no effect on OnUnitActiveSec= — and it matters more
      # here: the reviewer writes at 03:20 and this is a laptop that is
      # reliably asleep at 03:20. Without catch-up the night's proposals would
      # wait for whenever the machine next happens to be awake on a boundary.
      #
      # :07/:22/:37/:52 — a third offset, distinct from claude-pull (:0/10) and
      # claude-marketplace-pull (:5/10), so the three do not reach GitHub in
      # the same breath. Fifteen minutes rather than ten because a backup does
      # not need ten-minute freshness and each tick costs a network round trip.
      OnCalendar = "*:7/15";
      Persistent = true;
      AccuracySec = "1min";
      RandomizedDelaySec = "1min";
