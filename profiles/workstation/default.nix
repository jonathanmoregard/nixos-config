# Workstation profile: the daily-driver system minus the machine.
#
# Everything Dellan runs that does not depend on its hardware or its
# identity lives here, so a replacement laptop is a new hosts/<name>/
# that imports this profile plus its own hardware modules. Host files
# keep only:
#   - hardware-configuration.nix, swap sizing, the model module
#     (e.g. modules/nixos/dell-latitude-7440.nix);
#   - networking.hostName;
#   - age.rekey.hostPubkey + age.rekey.localStorageDir.
#
# Lives at profiles/workstation/default.nix (two levels deep, like a
# host file) so every `../../` path below, and the `rekeyFile` lines
# add-secret inserts, resolve the same as they did in hosts/dellan/.
{ config, pkgs, ... }:
{
  imports = [
    ...
    # only the pieces that MUST live on the workstation: pull-based
    # deploy + webhook latency optimization.
